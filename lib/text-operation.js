...
class TextOperation {
  ...
  insert(str) {
    ...
    this.targetLength += charLength(str);
    const ops = this.ops;
  ...
  apply(str) {
    ...
    if (charLength(str) !== operation.baseLength) {
      throw new Error(
    ...
    for (let i = 0, l = ops.length; i < l; i++) {
      const op = ops[i];
      if (isRetain(op)) {
        if (strIndex + op > charLength(str)) {
          throw new Error(
            "Operation can't retain more characters than are left in the string."
          );
        }
        // Copy skipped part of the old string.
        newStr.push(Array.from(str).slice(strIndex, strIndex + op).join(''));
        strIndex += op;
      } else if (isInsert(op)) {
        // Insert string.
        newStr.push(op);
  ...
  invert(str) {
    ...
    for (let i = 0, l = ops.length; i < l; i++) {
      ...
      if (isRetain(op)) {
        ...
        inverse.delete(charLength(op));
      } else {
        // delete op
        inverse.insert(Array.from(str).slice(strIndex, strIndex - op).join(''));
        strIndex -= op;
      }
    }
    return inverse;
  ...
  compose(operation2) {
    ...
    while (true) {
      ...
      if (isRetain(op1) && isRetain(op2)) {
        ...
        if (charLength(op1) > -op2) {
          op1 = Array.from(op1).slice(-op2).join('');
          op2 = ops2[i2++];
        } else if (charLength(op1) === -op2) {
          op1 = ops1[i1++];
          op2 = ops2[i2++];
        } else {
          op2 = op2 + charLength(op1);
          op1 = ops1[i1++];
        }
      } else if (isInsert(op1) && isRetain(op2)) {
        if (charLength(op1) > op2) {
          operation.insert(Array.from(op1).slice(0, op2).join(''));
          op1 = Array.from(op1).slice(op2).join('');
          op2 = ops2[i2++];
        } else if (charLength(op1) === op2) {
  ...
  shouldBeComposedWith(other) {
    ...
    if (isInsert(simpleA) && isInsert(simpleB)) {
      return startA + charLength(simpleA) === startB;
    }
  ...
  shouldBeComposedWithInverted(other) {
    ...
    if (isInsert(simpleA) && isInsert(simpleB)) {
      return startA + charLength(simpleA) === startB || startA === startB;
    }
...
function transform(operation1, operation2) {
  ...
  while (true) {
    ...
    if (isInsert(op1)) {
      ...
      operation2prime.retain(charLength(op1));
      op1 = ops1[i1++];
      continue;
    }
    if (isInsert(op2)) {
      operation1prime.retain(charLength(op2));
...
TextOperation.isRetain = isRetain;
TextOperation.isInsert = isInsert;
TextOperation.isDelete = isDelete;
TextOperation.fromJSON = fromJSON;
